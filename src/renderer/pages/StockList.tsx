...
import { Table, Typography, Alert, Input, Space, Tag, Switch, Button, Select } from 'antd';
import { SearchOutlined, ReloadOutlined } from '@ant-design/icons';
import type { ColumnsType } from 'antd/es/table';
import type { StockView, StockSummaryView } from '../../shared/types';
...
const StockList: React.FC = () => {
  ...
  const handleQuery = () => {
    const newFilter = { bookTitle: localBookTitle, page: 1, pageSize };
    setFilter({ bookTitle: localBookTitle });
    setPage(1, pageSize);
  ...
  const handleReset = () => {
    ...
    setFilter({ bookTitle: undefined });
    setPage(1, pageSize);
    const newFilter = { bookTitle: undefined, page: 1, pageSize };
    if (viewMode === 'detail') fetchStocks(newFilter); else fetchSummary(newFilter);
  };
  ...
  return (
    <div>
      ...
      {viewMode === 'detail' ? (
        <Table columns={detailColumns} dataSource={filteredStocks} rowKey="stockId" loading={loading} scroll={{ x: 1000 }} pagination={{ current: page, pageSize, total: localStatus ? filteredStocks.length : total, showSizeChanger: true, showTotal: (t) => `共 ${t} 条`, onChange: (p, ps) => { setPage(p, ps); fetchStocks({ page: p, pageSize: ps }); } }} />
      ) : (
