...
const OutboundList: React.FC = () => {
  ...
  const fetchRecords = useCallback(async (p?: number, ps?: number) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '获取出库记录失败');
    } finally {
      setLoading(false);
    }
  ...
  const columns: ColumnsType<OutboundRecord> = [
    {
      ...
      dataIndex: 'bookId',
      key: 'bookTitle',
      render: (id: string) => bookMap[id]?.title ?? id,
    },
    {
      title: '出库日期',
    ...
    },
    {
      ...
      render: (id: string) => {
        ...
        return l ? `${l.warehouse}-${l.shelf}-${l.layer}` : id;
      },
    },
    { title: '买家', dataIndex: 'buyer', key: 'buyer', render: (v: string | null) => v ?? '-' },
    {
      ...
      render: (_: unknown, record: OutboundRecord) => (
        <Space>
          ...
          <Popconfirm
            title="确认删除"
            description="删除出库记录将增加对应库存数量，确定要删除吗？"
    ...
    <div>
      ...
      <Space style={{ marginBottom: 16 }} wrap>
        <Select
          ...
          style={{ width: 200 }}
          value={filterBookId}
          onChange={(v) => { setFilterBookId(v); setPage(1); }}
        ...
        <Select
          ...
          onChange={(v) => { setFilterLocationId(v); setPage(1); }}
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
        <Button icon={<UnorderedListOutlined />} onClick={() => setBatchFormOpen(true)}>
          批量出库
        </Button>
