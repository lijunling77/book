...
const PriceHistory: React.FC = () => {
  ...
  useEffect(() => {
    bookApi.list({ page: 1, pageSize: 1000 }).then((r) => setBooks(r.data)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    if (selectedBookId && selectedEditionId) {
      setLoading(true);
      setError(null);
      Promise.all([
        priceApi.purchaseHistory(selectedBookId, selectedEditionId),
        priceApi.sellingHistory(selectedBookId, selectedEditionId),
        priceApi.stats(selectedBookId, selectedEditionId),
      ])
        .then(([ph, sh, st]) => {
          setPurchaseHistory(ph);
          setSellingHistory(sh);
          setStats(st);
        })
  ...
  return (
    <div>
      ...
      {!selectedBookId || !selectedEditionId ? (
        <Empty description="请选择书籍和版本以查看价格历史" />
      ) : loading ? (
          ...
          {stats && (
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
              <Col xs={24} sm={12} lg={6}>
                <Card><Statistic title="平均买入价格" value={priceDisplay(stats.averagePurchasePrice)} /></Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card><Statistic title="平均售出价格" value={priceDisplay(stats.averageSellingPrice)} /></Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card><Statistic title="最近买入价格" value={priceDisplay(stats.latestPurchasePrice)} /></Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card><Statistic title="最近售出价格" value={priceDisplay(stats.latestSellingPrice)} /></Card>
              </Col>
            </Row>
          )}
