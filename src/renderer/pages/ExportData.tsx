...
import type { Book, ExportFormat } from '../../shared/types';
import { exportApi, bookApi } from '../utils/ipc';
...
const ExportData: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      let filePath: string;
      if (exportType === 'inbound') filePath = await exportApi.inbound({ bookId: filterBookId, dateRange }, format);
      else if (exportType === 'outbound') filePath = await exportApi.outbound({ bookId: filterBookId, dateRange }, format);
      else if (exportType === 'stock') filePath = await exportApi.stock({ bookTitle: undefined }, format);
    ...
    <div>
      ...
      <Card>
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          <Space wrap>
            <div><div style={{ marginBottom: 4, fontWeight: 500 }}>导出类型</div><Select style={{ width: 200 }} value={exportType} onChange={(v) => { setExportType(v); setFilterBookId(undefined); setFilterDateRange(null); }} options={[{ value: 'inbound', label: '入库记录' }, { value: 'outbound', label: '出库记录' }, { value: 'stock', label: '库存信息' }, { value: 'profit', label: '利润统计' }]} /></div>
          ...
          <Space>
            ...
            <Button icon={<ReloadOutlined />} onClick={() => { setFilterBookId(undefined); setFilterDateRange(null); }}>重置</Button>
          </Space>
        </Space>
      </Card>
    </div>
