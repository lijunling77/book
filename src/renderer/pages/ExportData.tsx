...
const exportTypeOptions = [
  ...
  { value: 'outbound', label: '出库记录' },
  { value: 'stock', label: '库存信息' },
  ...
  const handleExport = async () => {
    ...
    try {
      const dateRange = filterDateRange
        ? { startDate: filterDateRange[0].format('YYYY-MM-DD'), endDate: filterDateRange[1].format('YYYY-MM-DD') }
        : undefined;

      let filePath: string;
      if (exportType === 'inbound') {
        filePath = await exportApi.inbound(
          { bookId: filterBookId, locationId: filterLocationId, dateRange },
          format,
        );
      } else if (exportType === 'outbound') {
        filePath = await exportApi.outbound(
          { bookId: filterBookId, locationId: filterLocationId, dateRange },
          format,
        );
      } else if (exportType === 'stock') {
        filePath = await exportApi.stock(
          { bookTitle: undefined, category: filterCategory, locationId: filterLocationId },
          format,
        );
      } else {
        filePath = await exportApi.profit(
    ...
    <div>
      ...
      <Card>
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          <Space wrap>
            <div>
              <div style={{ marginBottom: 4, fontWeight: 500 }}>导出类型</div>
              <Select
                style={{ width: 200 }}
                value={exportType}
                onChange={(v) => {
          ...
          <div>
            ...
            <Space wrap>
              {(exportType === 'inbound' || exportType === 'outbound') && (
                <Select
                  placeholder="按书籍筛选"
                  allowClear
                  showSearch
              ...
              {(exportType === 'inbound' || exportType === 'outbound' || exportType === 'stock') && (
                <Select
                  placeholder="按位置筛选"
                  allowClear
                  showSearch
              ...
              {(exportType === 'stock' || exportType === 'profit') && (
                <Select
                  placeholder="按分类筛选"
                  allowClear
                  showSearch
              ...
              {(exportType === 'inbound' || exportType === 'outbound' || exportType === 'profit') && (
                <RangePicker
