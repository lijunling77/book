...
import type { Book, Location, OutboundRecord, CreateOutboundInput, UpdateOutboundInput } from '../../shared/types';
...
const OutboundForm: React.FC<OutboundFormProps> = ({ open, record, onClose, onSuccess }) => {
  ...
  const handleOk = async () => {
    try {
      ...
      if (isEdit && record) {
        const updateData: UpdateOutboundInput = { locationId: values.locationId, outboundDate: dateStr, quantity: values.quantity, sellingPrice: values.sellingPrice, buyer: values.buyer || null };
        await outboundApi.update(record.id, updateData);
        message.success('出库记录更新成功');
      } else {
        const createData: CreateOutboundInput = { bookId: values.bookId, locationId: values.locationId, outboundDate: dateStr, quantity: values.quantity, sellingPrice: values.sellingPrice, buyer: values.buyer || null };
  ...
  return (
    <>
      <Modal title={isEdit ? '编辑出库记录' : '新增出库记录'} open={open} onOk={handleOk} onCancel={onClose} destroyOnClose width={600}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="outboundDate" label="出库日期" rules={[{ required: true, message: '请选择出库日期' }]}><DatePicker style={{ width: '100%' }} /></Form.Item>
          ...
          <Form.Item name="sellingPrice" label="售出价格（元）" rules={[{ required: true, message: '请输入售出价格' }, { type: 'number', min: 0, message: '价格不能为负' }]}><InputNumber min={0} step={0.01} precision={2} style={{ width: '100%' }} placeholder="请输入售出价格" /></Form.Item>
          <Form.Item name="buyer" label="买家"><Input placeholder="请输入买家（可选）" /></Form.Item>
        </Form>
      </Modal>
